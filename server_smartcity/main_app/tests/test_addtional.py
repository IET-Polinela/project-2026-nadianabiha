...
class MainAppMonolithicViewsCoverageTests(TestCase):
    ...
    def test_report_list_view_admin(self):
        ...
        response = self.client.get(reverse('report_list'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'main_app/report_page.html')
    ...
    def test_report_update_status_view_unauthenticated(self):
        response = self.client.post(reverse('update_report_status', kwargs={'pk': self.report.id}), {'status': 'VERIFIED'})
        self.assertEqual(response.status_code, 302)
